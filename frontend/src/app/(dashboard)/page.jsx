...
import MeetingCalendar from "@/components/dashboard/MeetingCalendar"; // <-- NEW IMPORT
...
export default function HomePage() {
  ...
  // Fetch meetings for the calendar on mount (NEW)
  useEffect(() => {
  ...
  return (
    // Removed justify-center and added overflow-y-auto so the page can scroll 
    // naturally when the calendar is added below the form.
    <main className="flex-1 flex flex-col items-center px-4 py-8 md:py-12 overflow-y-auto">
      <div className="w-full max-w-5xl mx-auto space-y-10">
        
        {/* ─── SECTION 1: Existing Meeting Form (Functionality Unchanged) ─── */}
        <div className="w-full max-w-lg mx-auto">
          <div className="text-center mb-8">
            ...
            {user?.name && (
              <p className="text-sm text-muted mt-2">Hi {user.name}</p>
            )}
          </div>
        ...
        {/* ─── SECTION 2: Meeting Activity Calendar (NEW) ─── */}
