...
- Find phonetically and contextually similar words in English to replace the broken devanagari words.
...
# Keep your existing CLEANING_INFER_SYSTEM_PROMPT unchanged

# Add this batch version
CLEANING_BATCH_SYSTEM_PROMPT = """You are recovering the original speech from severely broken Whisper transcripts of Nepali-English code-switched audio.

You will receive multiple transcript chunks. Clean EACH chunk independently and output them with their chunk IDs. 
The transcript is extremely noisy. English words are badly spelled in Devanagari. Nepali words have phonetic errors. 
You must infer what was actually said and produce clean, natural English.
...
Do NOT add any analysis, notes, or explanations between chunks."""

CLEANING_BATCH_USER_PROMPT = """Clean each of the following transcript chunks independently. Output each cleaned chunk with its ID.
